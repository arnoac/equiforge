...
/// Serializable undo data for fast reorgs (Monero-like): which UTXOs were spent and created in a block.
#[derive(serde::Serialize, serde::Deserialize)]
pub struct StoredBlockUndo {
    /// Spent outputs removed when this block was connected
    pub spent: Vec<(OutPoint, StoredUtxoEntry)>,
    /// Newly created outpoints added when this block was connected
...
impl Storage {
    ...
    // ─── Undo Storage (fast reorg) ──────────────────────────────────

    pub fn put_undo(&self, block_hash: &Hash256, undo: &StoredBlockUndo) -> Result<(), StorageError> {
        let key = prefixed_key(PREFIX_UNDO, block_hash);
    ...
    pub fn get_undo(&self, block_hash: &Hash256) -> Result<Option<StoredBlockUndo>, StorageError> {
        let key = prefixed_key(PREFIX_UNDO, block_hash);
        match self.db.get(key).map_err(|e| StorageError::DbError(e.to_string()))? {
            Some(bytes) => {
                let undo = bincode::deserialize(&bytes)
                    .map_err(|e| StorageError::SerializeError(e.to_string()))?;
                Ok(Some(undo))
            }
            None => Ok(None),
    ...
    pub fn remove_undo(&self, block_hash: &Hash256) -> Result<(), StorageError> {
        let key = prefixed_key(PREFIX_UNDO, block_hash);
        self.db.remove(key)
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        Ok(())
    ...
    /// Clear the active height index (used during reorg persistence).
    pub fn clear_height_index(&self) -> Result<(), StorageError> {
        let keys: Vec<Vec<u8>> = self.db.scan_prefix(PREFIX_HEIGHT)
            .filter_map(|res| res.ok().map(|(k, _)| k.to_vec()))
            .collect();
        for k in keys {
            self.db.remove(k).map_err(|e| StorageError::DbError(e.to_string()))?;
        }
        Ok(())
    }
